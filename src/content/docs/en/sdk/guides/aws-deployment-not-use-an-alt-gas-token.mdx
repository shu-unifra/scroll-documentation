...
section: sdk
date: Last Modified
title: "Amazon Web Services Deployment not use an alt-gas token"
lang: "en"
permalink: "sdk/guides/aws-deployment-not-use-an-alt-gas-token"
...
import brewtapweaveworkstap from "./_images/brew-tap-weaveworks-tap.png"
import awsinstalleksctl from "./_images/aws-install-eksctl.png"
import awscreateEKScluster from "./_images/aws-create-EKS-cluster.png"
import awscreateEKScluster2 from "./_images/aws-create-EKS-cluster2.png"
import awscreateEKScluster3 from "./_images/aws-create-EKS-cluster3.png"
import kubectl_get_nodes from "./_images/kubectl_get_nodes.png"
import selectamazonebscsidriver from "./_images/select-amazon-ebs-csi-driver.png"
import selectamazonebscsidriver2 from "./_images/select-amazon-ebs-csi-driver2.png"
import describenodegroup from "./_images/describe-nodegroup.png"
import attachrolepolicy from "./_images/attach-role-policy.png"
import set_default_storageclass from "./_images/set_default_storageclass.png"
import make_bootstrap from "./_images/make-bootstrap.png"
import make_install from "./_images/make-install.png"
import kubectl_get_services from "./_images/kubectl-get-services.png"
import aws_create_database1 from "./_images/aws_create_database1.png"
import aws_create_database2 from "./_images/aws_create_database2.png"
import aws_create_database3 from "./_images/aws_create_database3.png"
import aws_create_database4_0 from "./_images/aws_create_database4_0.png"
import aws_create_database4_1 from "./_images/aws_create_database4_1.png"
import aws_create_database5 from "./_images/aws_create_database5.png"
import aws_connection_info from "./_images/aws_connection_info.png"
import push_secrets1 from "./_images/push_secrets1.png"
import push_secrets2 from "./_images/push_secrets2.png"
import push_secrets3 from "./_images/push_secrets3.png"
import push_secrets4 from "./_images/push_secrets4.png"

This guide documents getting a Scroll SDK deployment working on AWS's (Amazon Web Services) Kubernetes and managed databases, using Cloudflare for DNS.
...
*Guide last updated October 14, 2024 and refers to example files from [this commit](TODO).*
</Aside>
...
```
<ClickToZoom src={brewtapweaveworkstap} alt="aws install eksctl" /> 
<ClickToZoom src={awsinstalleksctl} alt="aws install eksctl" /> 


#### 2. Create EKS Cluster
You need to select a region first. Here I use `us-east-1`. For more regions, see [aws's region list.](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.RegionsAndAvailabilityZones.html) 

This may take a while.
```
eksctl create cluster \
--name scroll-sdk-minimal \
--region us-east-1 \
--nodegroup-name standard-workers \
--managed
```
`scroll-sdk-minimal` is cluster name, you can change it as you want. Visit aws website to see clusters you have.
https://us-east-1.console.aws.amazon.com/eks/home?region=us-east-1#/clusters 

<ClickToZoom src={awscreateEKScluster} alt="aws create EKS cluster" /> 
{/* <ClickToZoom src={awscreateEKScluster2} alt="aws create EKS cluster" />  */}
<ClickToZoom src={awscreateEKScluster3} alt="aws create EKS cluster" /> 

#### 3. Configure kubectl
After the cluster is created, eksctl will automatically configure kubectl for you. However, you can manually update your kubeconfig file using:
```
aws eks --region us-east-1 update-kubeconfig --name scroll-sdk-minimal
```
{/* <ClickToZoom src={kubectl_get_nodes} alt="kubectl get nodes" />  */}

#### 4. Verify Configuration
Verify that kubectl is correctly configured to communicate with your EKS cluster:
```
kubectl get nodes
```
This should display the worker nodes in your cluster.
<ClickToZoom src={kubectl_get_nodes} alt="kubectl get nodes" /> 

#### 5. Install aws-ebs-csi-driver

1. visit https://us-east-1.console.aws.amazon.com/eks/home?region=us-east-1#/clusters/scroll-sdk-minimal/create-add-ons

2. select `Amazon EBS CSI Driver`
<ClickToZoom src={selectamazonebscsidriver} alt="select `Amazon EBS CSI Driver`" /> 

3. at the bottom of this page, click `Next`
<ClickToZoom src={selectamazonebscsidriver2} alt="Install aws-ebs-csi-driver" /> 

4. Create a new file named `ebs-policy.json` with the following content.
...
An error occurred (EntityAlreadyExists) when calling the CreatePolicy operation: A policy called EKSNodeEBSPolicy already exists. Duplicate names are not allowed.
```

```
# check nodegroup role
~ $aws eks describe-nodegroup --cluster-name scroll-sdk-minimal --nodegroup-name standard-workers --query "nodegroup.nodeRole" --output text

arn:aws:iam::765480043105:role/eksctl-scroll-sdk-minimal-nodegrou-NodeInstanceRole-ESgLVwEFfPac
```
<ClickToZoom src={describenodegroup} alt="describe-nodegroup" /> 

- this is YOUR_ARN_NAME `arn:aws:iam::765480043105`
- this is YOUR_ROLE_NAME `eksctl-scroll-sdk-minimal-nodegrou-NodeInstanceRole-ESgLVwEFfPac` 

```
aws iam attach-role-policy --role-name YOUR_ROLE_NAME --policy-arn YOUR_ARN_NAME:policy/EKSNodeEBSPolicy
```
In this example:
<ClickToZoom src={attachrolepolicy} alt="attach-role-policy" /> 

#### 6. Set Default StorageClass 
```
~ $kubectl get sc
NAME   PROVISIONER             RECLAIMPOLICY   VOLUMEBINDINGMODE      ALLOWVOLUMEEXPANSION   AGE
gp2    kubernetes.io/aws-ebs   Delete          WaitForFirstConsumer   false                  112m
~ $kubectl patch storageclass gp2 -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"true"}}}'
storageclass.storage.k8s.io/gp2 patched
```
<ClickToZoom src={set_default_storageclass} alt="set-default-storageclass" /> 
...
# login with your github key
# echo your_key | docker login ghcr.io -u your_name --password-stdin

make bootstrap
make install
```
<ClickToZoom src={make_bootstrap} alt="make bootstrap" /> 
<ClickToZoom src={make_install} alt="make install" /> 
Check the service status:
```
kubectl get services
```
<ClickToZoom src={kubectl_get_services} alt="kubectl get services" /> 

#### 8. Clean Up (When No Longer Needed)
To delete the cluster and avoid unnecessary costs:
```
eksctl delete cluster --name scroll-sdk-minimal --region us-east-1
```
#### 9. https config */}
...
<ClickToZoom src={aws_create_database1} alt="" />
select [standard create]
<ClickToZoom src={aws_create_database2} alt="" />
select [self managed]
<ClickToZoom src={aws_create_database3} alt="" />
Now choose the same VPC as your cluster. If you are not sure which one it is, go back to your cluster and check the VPC information there.

<ClickToZoom src={aws_create_database4_0} alt="" />
<ClickToZoom src={aws_create_database4_1} alt="" />
Now click [Create Database] and wait for it to finish.
<ClickToZoom src={aws_create_database5} alt="" />
...
After the cluster is created, eksctl will automatically configure kubectl for you.
Verify that kubectl is correctly configured to communicate with your EKS cluster:
```
kubectl get nodes
```
...
Return to AWS and get your admin connection information:

<ClickToZoom src={aws_connection_info} alt="Aws Connection Information" />
...
### Dealing with Connections & Connection Pools (TODO)
...
Lastly, we need to take the configuration values that are sensitive and publish them to wherever we're deploying "secrets". AWS provides a Secret Management tool. To allow your cluster to access it, you need to configure some permissions.

1.  Find your cluster on the AWS webpage

     https://us-east-1.console.aws.amazon.com/eks/home?region=us-east-1#/clusters
2. Click your cluster name→Click **Compute**
3. Now you can see node groups, in this sample, **standard-workers**
<ClickToZoom src={push_secrets1} alt="" /> 
4. click group name **standard-workers**.
5. In **Details** tab, click **View in IAM**
<ClickToZoom src={push_secrets2} alt="" /> 
6. Click **Add permissions** -> **Attach policies**
<ClickToZoom src={push_secrets3} alt="" /> 
7. Type "secretsmanager" in the search box, and you will see `SecretsManagerReadWrite`, check the box next to it.
8. Then click button **Add Permissions.**
<ClickToZoom src={push_secrets4} alt="" /> 

Next, run `scrollsdk setup push-secrets` and select `AWS`. 
